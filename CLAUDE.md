...
- `.claude/freshrss_utils.py` — GReader API helpers: authenticatie, stream-fetch, auto-sterren; leest credentials uit `~/bin/.researchvault-env`. `freshrss_fetch_stream()` geeft sinds 19 aug 2026 een `StreamResult` met status `ok`/`leeg`/`mislukt`/`timeout` — dezelfde vorm als `feedreader_fetch.fetch_feed()`, en om dezelfde reden: `except Exception: return {}` maakte een HTTP 400 ononderscheidbaar van een lege stream, waardoor signaal 3 droogstond. `freshrss_read_stream()` leidt de read-state af uit de reading-list (`categories`), want de directe read-stream geeft bij FreshRSS 400. Opener injecteerbaar → `test_freshrss_utils.py` draait op kale stdlib
...
- `.claude/feedreader_labels.py` — pure labellogica over logregels, alleen stdlib en dus testbaar (`test_feedreader_labels.py`, inclusief de doc-contracttabel als testgevallen). Drie functies: `apply_skips()` (👎 markeren op identity vóór URL, en melden wat niet matchte), `mark_auto_starred()` (sterren die de pijplijn zichzelf gaf) en `split_positives()` (menselijk oordeel scheiden van zelfbevestiging). Bestaat omdat `feedreader-learn.py` een hyphen draagt en dus niet importeerbaar is — daardoor was de kern van de leerloop de enige onteste laag van de pijplijn
...
**Signaal 3 staat óók droog, en dubbel (vastgesteld 17 aug 2026).** `freshrss_read_urls()` bevraagt de stream `user/-/state/com.google/read`; FreshRSS antwoordt daarop met `HTTP 400 Bad Request`, wat door `except Exception: return {}` in `freshrss_fetch_stream()` een lege set werd. Werkende routes zijn wél beschikbaar — items uit `reading-list` die `user/-/state/com.google/read` in `categories` dragen (één call, URL zit erbij), of `stream/items/ids?s=…/read` (alleen IDs). Maar repareren daarvan verschuift slechts ~6 labels, want signaal 1 (ster) staat vóór signaal 3 in de labellus en sluit met `continue` af: van de 215 gelezen items zijn er 194 óók gesterd. Daardoor zijn álle negatieven in de dataset timeout-negatieven. Beide oorzaken en de meetopzet staan in het plan hieronder; niet los repareren.

> **Openstaande ontwerpvraag.** 1.745 van de 1.792 positieven komen via de FreshRSS-ster, maar items met score ≥`THRESHOLD_STAR` worden door de pijplijn zelf auto-gesterd. Het logboek onderscheidt een auto-ster niet van een handmatige, dus de leerloop bevestigt mogelijk grotendeels zijn eigen oordeel — het advies komt niet toevallig uit op precies `THRESHOLD_STAR`. Plan: `ResearchVault-plans/plans/leerloop-autoster-circulariteit.md`.
