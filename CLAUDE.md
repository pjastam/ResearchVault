...
- `.claude/feedreader_fetch.py` — ophaallaag met herkansing. `fetch_feed()` onderscheidt drie uitkomsten: `ok` (items aanwezig), `leeg` (feed-titel maar geen items — een echt lege feed) en `mislukt` (géén titel én géén items, dus de XML is niet geparsed). Alleen `mislukt` wordt herkanst (`FETCH_RETRIES = 1`, `FETCH_RETRY_DELAY = 2.0`); feeds met een gezette `bozo`-vlag maar bruikbare items gelden als geslaagd. Aanleiding: `feedparser.parse()` werpt geen exceptie bij een transiënte netwerkfout maar geeft stil een leeg object terug, waardoor mislukte fetches jarenlang als "0 items" in het log verdwenen. De parser is injecteerbaar en `feedparser` wordt lazy geïmporteerd, zodat `test_feedreader_fetch.py` op kale stdlib draait (CI heeft geen pip-install-stap). **Let op:** `import feedparser` in `feedreader-score.py` is daar niet lokaal in gebruik maar moet blijven staan — `backfill-scout.py` benadert het via `fr.feedparser`
- `.claude/test_feedreader_fetch.py` — unittests voor de ophaallaag (nep-parser, geen netwerk); draait mee in de `tests`-workflow
