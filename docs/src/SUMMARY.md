# Summary

[Introduction](introduction.md)

# Installation

- [Step 1: Homebrew](installation/homebrew.md)
- [Step 2: Zotero](installation/zotero.md)
- [Step 3: Python environment](installation/python.md)
- [Step 4: Zotero MCP](installation/zotero-mcp.md)
- [Step 5: Claude Code](installation/claude-code.md)
- [Step 6: Ollama](installation/ollama.md)
- [Step 7: Obsidian](installation/obsidian.md)
- [Step 8: Connect everything](installation/connect.md)
- [Step 9: First test](installation/first-test.md)

# Extensions

- [Step 10: yt-dlp & semantic search](extensions/yt-dlp.md)
- [Step 11: Podcast integration](extensions/podcasts.md)
- [Step 12: RSS integration + Phase 0 filtering](extensions/rss.md)
- [Step 13: Spaced repetition](extensions/spaced-repetition.md)
- [Step 14: Filter layer per source](extensions/filter-layer.md)
- [Step 15: Future perspective — local orchestrator](extensions/future-orchestrator.md)
- [Step 16: iPad access via SSH terminal](extensions/ipad-ssh.md)
- [Step 17: In-browser terminal](extensions/browser-terminal.md)

# Reference

- [Activating the skill & daily workflow](reference/daily-workflow.md)
- [Troubleshooting](reference/troubleshooting.md)
- [Privacy overview](reference/privacy.md)
