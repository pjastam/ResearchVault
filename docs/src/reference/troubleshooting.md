...
| launchd update not running | Nightly daemon did not fire (check `~/Library/Logs/nachtelijke-taken.log`) | Run `update-zotero` manually; verify the daemon is loaded with `sudo launchctl list \| grep nachtelijke`; if empty after reboot, see "Daemons not loading after reboot" below |
| Daemons not loading after reboot | macOS Background Task Management (BTM) blocking bash processes | Go to System Settings → General → Login Items & Extensions → Allow in the Background → enable `bash (4×)`, `sh (1×)`, `ollama (1×)`; diagnose with: `log show --predicate 'eventMessage contains "com.example"' --last boot \| grep disallowed` |
| Nightly daemon skipped despite Mac being on | Race condition: Mac woke up less than 5 minutes before the 06:00 trigger — `UserEventAgent-System` was not ready in time, launchd silently skipped the event | Verify wake is set to 05:55: `sudo pmset -g sched`; if missing, run `sudo pmset repeat wakeorpoweron MTWRFSU 05:55:00`; kick off manually with `sudo launchctl kickstart system/com.example.nachtelijke-taken` |
...
| FreshRSS container not running | Container stopped on HA Green | SSH in: `ssh hassio@<HA-GREEN-TAILNET-IP> "docker restart freshrss"`; verify with `ssh hassio@<HA-GREEN-TAILNET-IP> "docker ps \| grep freshrss"` |
