...
| Daemons not loading after reboot | macOS Background Task Management (BTM) blocking bash processes | Go to System Settings → General → Login Items & Extensions → Allow in the Background → enable `bash (4×)`, `sh (1×)`, `ollama (1×)`; diagnose with: `log show --predicate 'eventMessage contains "nl.pietstam"' --last boot \| grep disallowed` |
