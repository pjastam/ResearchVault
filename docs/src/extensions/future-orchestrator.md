# Step 14: Future perspective — local orchestrator

The only component in this stack that does not run fully locally is Claude Code as orchestrator. All generation tasks (bundle ingestion, wiki drafts, concept pages, syntheses) already run locally: olw drives mistral-small:22b on Ollama for the heavy generation, with qwen3.5:9b as a fallback for Phase-2 previews (`summarize_item.py`) — fully local, fully private. What goes through the Anthropic API are the prompts with which Claude Code steers the workflow: the intake, the phase monitoring, the vault conventions, the iterative Go/No-go dialogue, and the olw review gate before publication to `wiki/`.
...
**Advantages:** mature interface, actively maintained, works on macOS without Docker, mistral-small:22b is compatible with tool use in this configuration.
...
With a local orchestrator, the same logic must be passed as a system prompt to a local model such as mistral-small:22b. Instruction-following in complex multi-step workflows is noticeably less reliable with local models than with Claude Sonnet — not due to a lack of language capability, but because of consistency across multiple rounds and tools. The result is achievable, but requires considerable extra work for a less robust outcome.

This is also precisely why Claude Code structurally distinguishes itself as an orchestrator from local alternatives: not in raw generation quality (for which olw driving mistral-small:22b is already strong enough for most tasks), but in the reliability of workflow logic across phases and tools. Whether and when local models will reach this level is an open question. The landscape is changing fast — Open WebUI, ollmcp, and similar tools are actively in development and worth continuing to follow.
