...
| Phase | Goal | Approach |
|---|---|---|
| **1 — Cast wide** | Capture everything relevant | Items flow into Zotero `_inbox` from three sources: (1) `feedreader-score.py` scores RSS/YouTube/podcast feeds daily and produces a sorted HTML reader and Atom feeds; (2) items shared directly via the iOS share sheet; (3) manual additions from desktop/email/notes |
...
## Step 12: RSS integration (feedreader + NetNewsWire + FreshRSS sync)

In the 3-phase model, the feedreader is part of phase 1: it automatically scores all RSS items on relevance before you see them, so that `_inbox` only receives items you have consciously chosen. Only high-scoring items appear in the HTML reader and Atom feed. NetNewsWire reads those feeds and synchronises read/unread status across Mac, iPad, and iPhone via FreshRSS.

### 12a. Feedreader setup

The feedreader consists of three Python scripts and a list of feed URLs:

- `.claude/feedreader-list.txt` — list of feed URLs (one per line, `#` = comment)
- `.claude/feedreader-score.py` — fetches feeds, scores items, writes `filtered.xml` and `filtered.html`
- `.claude/feedreader-server.py` — local HTTP server on port 8765; serves the HTML reader and Atom feeds; handles 👎-clicks via `POST /skip`; generates article summaries via Ollama
- `.claude/feedreader-learn.py` — processes skip-queue and Zotero additions; gives threshold advice

These files are already present in the vault after cloning the repository.

**Add feeds** by appending URLs to `.claude/feedreader-list.txt`:

```
# ── Health policy ────────────────────────────
https://www.bmj.com/rss/ahead-of-print.xml
https://pubmed.ncbi.nlm.nih.gov/rss/search/?term=substitutiezorg&format=abstract
```
...
Open the result in your browser: `http://localhost:8765/filtered.html`

The HTML reader shows items sorted by score, with type filter buttons **Alles / 📄 / ▶️ / 🎙️** and a **⌨️ terminal** button that opens a ttyd terminal as iframe (see step 12d).

The feedreader produces four separate Atom feeds:
- `http://[mac-ip]:8765/filtered.xml` — all scored items combined
- `http://[mac-ip]:8765/filtered-webpage.xml` — web articles only
- `http://[mac-ip]:8765/filtered-youtube.xml` — YouTube videos only
- `http://[mac-ip]:8765/filtered-podcast.xml` — podcast episodes only

### 12b. Feedreader launchd agents

Three launchd agents keep the feedreader running automatically:

| Agent | Schedule | Function |
|---|---|---|
| `nl.researchvault.feedreader-server` | Always active | HTTP server on port 8765 |
| `nl.researchvault.feedreader-score` | Daily at 06:00 | Score run — fetches feeds and updates filtered.html |
| `nl.researchvault.feedreader-learn` | Daily at 06:15 | Learning loop — processes skip-queue and Zotero additions |

Create the plist files in `~/Library/LaunchAgents/`. Example for the server (repeat similarly for score and learn):
...
### 12c. ttyd — browser terminal

The **⌨️ terminal** button in the HTML reader opens a ttyd terminal as iframe. This lets you run Claude Code from within the browser on iPad or iPhone, without a separate SSH app.
...
**Create launchd agent** `~/Library/LaunchAgents/nl.researchvault.ttyd.plist`:
...
<dict>
  <key>Label</key>
  <string>nl.researchvault.ttyd</string>
  <key>ProgramArguments</key>
  <array>
...
The terminal is accessible at `http://[mac-ip]:7681` and as an iframe within the HTML reader.

> **Security note:** ttyd runs with `--writable` on your local network only. Do not expose port 7681 to the internet.

### 12d. FreshRSS — read/unread sync across devices
...
> **Note:** FreshRSS may show a "Niet ingedeeld" (Uncategorised) folder in NetNewsWire alongside any custom categories. This is a known limitation of the Google Reader API integration — the folder is empty and does not affect functionality. Organise feeds into folders within NetNewsWire itself.
...
**Recommended feeds to add to feedreader-list.txt:**
...
> **Phase 1 → phase 2:** Items in the feedreader have not yet been saved — they only appear in the filtered HTML reader. This is intentional: you browse through them and scan headlines and intros without immediately archiving anything. Only what is truly relevant gets forwarded to the vault. That is the filter moment.
...
> **Privacy note:** NetNewsWire and FreshRSS store feed data locally on the Mac mini. No reading habits are sent to external servers.
...
| Phase | What |
|------|-----|
| Phase 1 (automatic pre-filter) | `feedreader-score.py` scores all feeds daily; only relevant items appear in the HTML reader at `http://[mac-ip]:8765/filtered.html` |
| Phase 1 (capture) | NetNewsWire — reads filtered Atom feeds via FreshRSS; read/unread status syncs across Mac, iPad, iPhone |
| Filter moment | Scan headline and intro in the feedreader HTML reader or in NetNewsWire |
...
| No-go | Mark item as read in NetNewsWire, or click 👎 in the HTML reader to send an explicit negative signal to the learning loop |
...
| NetNewsWire | ✅ Fully | RSS data stored locally, no cloud account |
...
4. The first connection may ask you to accept a host key fingerprint — confirm it
...
Termius typically accepts the host key silently. To verify that the correct key was stored, run the following on the Mac mini to list the fingerprints of all host keys:

```bash
for f in /etc/ssh/ssh_host_*_key.pub; do ssh-keygen -lf "$f"; done
```

Compare the fingerprint shown in Termius (**Edit host → Known Host**) with the output above. The ED25519 fingerprint is the most commonly used in modern connections. If they match, the connection is verified.
...
You are now in exactly the same environment as when using Claude Code in VS Code on the Mac mini — including Zotero MCP, Ollama, whisper.cpp, and all workflow skills. Type `/research` to start the research workflow.
...
*Installation guide version 1.13 — April 2026*
